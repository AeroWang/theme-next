...
export default function ThemeSwitch() {
  ...
  return (
    <div className={clsx('relative h-10 w-10 cursor-pointer')} onClick={toggleTheme}>
      <IconSun
        className={clsx(
          'absolute left-1/2 top-1/2 h-3/5 w-3/5 -translate-x-1/2 -translate-y-1/2 transition-all duration-75',
          resolvedTheme === 'light' ? 'visible -rotate-45 opacity-100' : 'invisible rotate-0 opacity-0',
        )}
      />
      <IconMoon
        className={clsx(
          'absolute left-1/2 top-1/2 h-[55%] w-[55%] -translate-x-1/2 -translate-y-1/2 transition-all duration-75',
